...
#define CHECK(condition) do { \
    ...
    // partner embeds its RAM, so keeping every fixture in this function on
    // the stack exceeds the default 1 MiB stack of Windows executables.
    const auto make_emu = [] { return std::make_unique<partner_sio_test>(); };
    ...
    auto systems_burst_emu = make_emu();
