...
int main()
{
    ...
    const fs::path tmp_dir = root / "tests" / ".tmp-partos-gdp-shell-edit";
    ...
    send_text(emu, std::string("abc") + char(0x08) + char(0x08) + "d\r");
    if (!wait_for_next_prompt(emu, prompt_row, rows)) {
        std::puts("FAIL backspace command did not return");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (!rows_contain(rows, "hd0> ad") || rows_contain(rows, "hd0> abc d")) {
        std::puts("FAIL backspace erase was not rendered correctly");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    send_text(emu, std::string("xyz") + char(0x7F) + char(0x7F) + "q\r");
    if (!wait_for_next_prompt(emu, prompt_row, rows)) {
        std::puts("FAIL delete command did not return");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (!rows_contain(rows, "hd0> xq") || rows_contain(rows, "hd0> xyz q")) {
        std::puts("FAIL delete erase was not rendered correctly");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    cleanup();
    std::puts("test_partos_gdp_shell_edit: PASS");
    return 0;
}
