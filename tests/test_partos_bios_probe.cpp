...
static bool wait_for_boot_halt(partner &emu)
{
...
static std::vector<std::string> read_device_names(partner &emu, uint16_t dev_list_addr)
{
...
static std::vector<std::string> boot_and_probe(const std::filesystem::path &rom_path,
                                               ...
                                               bool &boot_ok)
{
    ...
    boot_ok = wait_for_boot_halt(emu);
    if (!boot_ok)
        return {};
    return read_device_names(emu, dev_list_addr);
}
                               ...
                               const char *label)
{
    int fails = 0;
    bool boot_ok = false;
    const auto names = boot_and_probe(rom_path, dev_list_addr, fd0_path, fd1_path, hdd_path, cfg, boot_ok);
...
{
    int fails = 0;
    const std::vector<std::string> base_names = { "ttyS0", "ttyS1", "ttyS2", "ttyS3", "rtc", "nvram" };
    ...
    const std::filesystem::path tmp_dir = root / "tests" / ".tmp-partos-probe";
    ...
    fails += expect_probe_result(
        ...
        scenario{ false, false, false, base_names }, "no_media");

    fails += expect_probe_result(
        rom_path, *dev_list_addr, fd0_path, fd1_path, hdd_path,
        scenario{ true, true, false, { "ttyS0", "ttyS1", "ttyS2", "ttyS3", "rtc", "nvram", "fd0", "fd1" } }, "two_floppies");

    fails += expect_probe_result(
        rom_path, *dev_list_addr, fd0_path, fd1_path, hdd_path,
        scenario{ false, false, true, { "ttyS0", "ttyS1", "ttyS2", "ttyS3", "rtc", "nvram", "sda" } }, "hard_disk");

    fails += expect_probe_result(
        rom_path, *dev_list_addr, fd0_path, fd1_path, hdd_path,
        scenario{ true, true, true, { "ttyS0", "ttyS1", "ttyS2", "ttyS3", "rtc", "nvram", "fd0", "fd1", "sda" } }, "combined");

    if (fails == 0) {
        std::puts("test_partos_bios_probe: PASS");
        return 0;
    }

    std::printf("test_partos_bios_probe: %d failure(s)\n", fails);
    return 1;
}
