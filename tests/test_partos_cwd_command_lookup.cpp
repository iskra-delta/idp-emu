                          ...
                          std::vector<std::string> &rows_out)
{
    for (uint64_t i = 0; i < COMMAND_TICK_LIMIT; ++i) {
        ...
        if ((prompt_row > prev_prompt_row) &&
            !rows_out[(size_t)prompt_row].empty() &&
            (rows_out[(size_t)prompt_row].back() == '>')) {
            return true;
        }
    }
...
int main()
{
    ...
    const fs::path tmp_dir = root / "tests" / ".tmp-partos-cwd-command-lookup";
    ...
    const std::string dir_name = "testcmd";
    ...
    prompt_row = find_last_prompt_row(rows);
    send_text(emu, "cp mem.com " + dir_name + "/memory.com\r");
    if (!wait_for_next_prompt(emu, prompt_row, rows)) {
    ...
    if (rows_contain(rows, "?")) {
        std::puts("FAIL cp reported an error for slash destination");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    prompt_row = find_last_prompt_row(rows);
    send_text(emu, dir_name + "/memory\r");
    if (!wait_for_next_prompt(emu, prompt_row, rows)) {
        std::puts("FAIL slash command path did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (!rows_contain(rows, "SYS used=") || !rows_contain(rows, "USR used=")) {
        std::puts("FAIL slash command path did not execute copied mem.com");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (rows_contain(rows, "?")) {
        std::puts("FAIL slash command path reported an error");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    if (!rows_contain(rows, "hd0/" + dir_name + ">")) {
        std::puts("FAIL prompt did not reflect cwd after cd");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (rows_contain(rows, "?")) {
        std::puts("FAIL cd reported an error");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    prompt_row = find_last_prompt_row(rows);
    send_text(emu, "memory\r");
    if (!wait_for_next_prompt(emu, prompt_row, rows)) {
        std::puts("FAIL cwd-local command did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (rows_contain(rows, "?")) {
        std::puts("FAIL cwd-local command reported an error");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    send_text(emu, "mem\r");
    if (!wait_for_next_prompt(emu, prompt_row, rows)) {
        std::puts("FAIL root leak probe did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (find_last_prompt_row(rows) >= (prompt_row + 3)) {
        std::puts("FAIL root mem.com still executed from subdirectory");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    cleanup();
    std::puts("test_partos_cwd_command_lookup: PASS");
    return 0;
}
