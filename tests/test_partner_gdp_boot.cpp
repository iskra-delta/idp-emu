...
int main()
{
    ...
    ok &= check(emu.get_ef9367().y == 100u,
                "EF9367 X-home command destroyed the banner Y coordinate");

    auto frame = std::make_unique<display>();
    emu.render_to(*frame);
    const size_t expected_band = lit_pixels(*frame, 220, 420);
    const size_t wrong_top_band = lit_pixels(*frame, 0, 120);
    const size_t wrong_bottom_band = lit_pixels(*frame, 420, display::FB_H);
    ok &= check(expected_band > 30000u,
                "rendered banner is absent from its ROM-defined screen band");
    ok &= check(wrong_top_band == 0u && wrong_bottom_band == 0u,
                "rendered banner is vertically displaced or clipped");

    bool key_sent = false;
    bool key_consumed = false;
    while (emu.get_tick_count() < BOOT_INPUT_LIMIT) {
        emu.tick();
        if (!key_sent && boot_prompt_wait(emu.get_current_pc())) {
