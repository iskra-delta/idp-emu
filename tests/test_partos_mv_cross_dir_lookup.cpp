...
bool rows_contain_after(const std::vector<std::string> &rows,
                        int start_row,
                        const std::string &needle)
{
    for (int row = std::max(0, start_row + 1); row < (int)rows.size(); ++row) {
        if (rows[(size_t)row].find(needle) != std::string::npos)
            return true;
    }
...
bool wait_for_top_prompt(partner_gdp &emu,
                         uint64_t tick_limit,
                         std::vector<std::string> &rows_out)
{
    for (uint64_t i = 0; i < tick_limit; ++i) {
        ...
        rows_out = visible_rows(emu.get_avdc());
        if (!rows_out.empty() && is_prompt_row(rows_out[0]) &&
            (find_last_prompt_row(rows_out) == 0)) {
            return true;
        }
    }
...
int main()
{
    ...
    const fs::path tmp_dir = root / "tests" / ".tmp-partos-mv-cross-dir-lookup";
    ...
    const std::string dir_name = "mvtest";
    ...
    if (rows_contain(rows, "?")) {
        std::puts("FAIL mkdir reported an error");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    if (!wait_for_command_return(emu, "cd " + dir_name, rows)) {
        std::puts("FAIL cd into move dir did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (!rows_contain(rows, "hd0/" + dir_name + ">")) {
        std::puts("FAIL prompt did not enter move dir");
        dump_rows(rows);
        cleanup();
        return 1;
    }

    send_text(emu, "mv clear.com c.com\r");
    if (!wait_for_command_return(emu, "mv clear.com c.com", rows)) {
        std::puts("FAIL failed mv probe did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (!rows_contain_after(rows, find_row_with_text(rows, "mv clear.com c.com"), "?")) {
        std::puts("FAIL failed mv probe did not report an error");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    send_text(emu, "mv ../clear.com c.com\r");
    if (!wait_for_command_return(emu, "mv ../clear.com c.com", rows)) {
        std::puts("FAIL cross-dir mv did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (rows_contain_after(rows, find_row_with_text(rows, "mv ../clear.com c.com"), "?")) {
        std::puts("FAIL cross-dir mv reported an error");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    send_text(emu, "sl\r");
    if (!wait_for_command_return(emu, "sl", rows)) {
        std::puts("FAIL missing-command probe did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    send_text(emu, "ls\r");
    if (!wait_for_command_return(emu, "ls", rows)) {
        std::puts("FAIL ls in move dir did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (!rows_contain(rows, "c.com")) {
        std::puts("FAIL moved file not visible in target dir");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    send_text(emu, "cd ..\r");
    if (!wait_for_command_return(emu, "cd ..", rows)) {
        std::puts("FAIL cd back to root did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (!rows_contain(rows, "hd0>")) {
        std::puts("FAIL prompt did not return to root");
        dump_rows(rows);
        cleanup();
        return 1;
    }

    send_text(emu, dir_name + "/c\r");
    if (!wait_for_top_prompt(emu, COMMAND_TICK_LIMIT, rows)) {
        std::puts("FAIL moved file path did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (rows_contain(rows, "?")) {
        std::puts("FAIL moved clear.com clone reported an error");
        dump_rows(rows);
        cleanup();
        return 1;
    }

    const int before_root_probe = find_last_prompt_row(rows);
    send_text(emu, "c\r");
    if (!wait_for_command_return(emu, "c", rows)) {
        std::puts("FAIL root lookup probe did not return to shell");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    if (find_last_prompt_row(rows) >= (before_root_probe + 3)) {
        std::puts("FAIL root unexpectedly executed moved file from subdir");
        dump_rows(rows);
        cleanup();
        return 1;
    }
    ...
    cleanup();
    std::puts("test_partos_mv_cross_dir_lookup: PASS");
    return 0;
}
