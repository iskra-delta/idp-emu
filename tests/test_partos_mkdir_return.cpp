...
constexpr uint64_t SETTLE_TICKS = 2'000'000ULL;
...
bool file_contains_ascii(const std::filesystem::path &path,
                         const std::string &needle)
{
...
int main()
{
    ...
    const fs::path tmp_dir = root / "tests" / ".tmp-partos-mkdir-return";
    ...
    const std::string dir_name = "MKDRET1";
    std::error_code ec;
...

    {
        ...
        for (uint64_t i = 0; i < SETTLE_TICKS; ++i)
            emu.tick();
        rows = visible_rows(emu.get_avdc());

        const int cmd_row = find_row_with_text(rows, "mkdir " + dir_name);
        const int prompt_row = find_last_prompt_row(rows);

        if (cmd_row < 0) {
            std::puts("FAIL mkdir command line not visible");
            dump_rows(rows);
            cleanup();
            return rc;
        }

        if (prompt_row <= cmd_row) {
    ...
    if (!file_contains_ascii(hdd_path, dir_name)) {
        std::puts("FAIL mkdir did not persist a directory entry");
        cleanup();
        return rc;
    }

    cleanup();
    std::puts("test_partos_mkdir_return: PASS");
    return 0;
}
