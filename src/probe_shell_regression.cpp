...
//   IDP_STEP_BUDGET=N  max ticks to wait for a prompt return  (default 15e6)
//
...
constexpr uint64_t DEFAULT_STEP_BUDGET = 15'000'000ULL;
...
static bool prompt_returned(const partner_crt &emu)
{
    return text_ends_at_prompt(emu.dump_raw_serial_text()) ||
           text_ends_at_prompt(emu.dump_terminal_text());
}
...
static step_result run_step(partner_crt &emu,
                            const step &s,
                            uint64_t key_ticks,
...
int main(int argc, char **argv)
{
    ...
    std::printf("PartOS shell regression pack: %zu steps "
                "(key_ticks=%llu step_budget=%llu%s)\n\n",
                script.size(),
